...
{
    public partial class DoanhThuTheoNuoc : Form
    {
        public DoanhThuTheoNuoc()
        {
            InitializeComponent();
        }

        private void DoanhThuTheoNuoc_Load(object sender, EventArgs e)
        {
            ReportBLL report = new ReportBLL();
            DataTable dt = report.GetDoanhThuTheoNuoc();
            ...
            {
                arrays.Add(dt.Rows[i]["tennuocsx"].ToString());
            ...
            this.chart1.Titles.Add("Doanh thu theo từng nước");
            for (int i = 0; i < seriesArray.Length; i++)
            {
                // Add series.
                Series series = this.chart1.Series.Add(seriesArray[i]);
                this.chart1.Series[seriesArray[i]].SmartLabelStyle.Enabled = true;
                this.chart1.Series[seriesArray[i]].AxisLabel = "Nước sx";
